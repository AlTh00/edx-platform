...
class VerifiedUpgradeDeadlineDate(DateSummary):
    ...
    def date(self):
        ...
        if self.course and self.course_overview.self_paced and self.enrollment:
            ...
            # NOTE: We check the pk here to determine if we have an instance generated from the current() method or
            # pulled from the database. If the instance isn't from the database, it is not relevant.
            if (not course_config.pk) or (course_config.enabled and not course_config.opt_out):
                ...
                if global_config.enabled:
                    ...
                    # If the course has an override, use it.
                    ...
                    user_deadline = start + datetime.timedelta(days=delta)
